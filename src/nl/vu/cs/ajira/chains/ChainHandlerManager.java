...
public class ChainHandlerManager {
	...
	static final Logger log = LoggerFactory.getLogger(ChainHandlerManager.class);

	private static ChainHandlerManager manager = new ChainHandlerManager();

	private Context context;
	private ChainTerminator terminator;
	...
	public void setContext(Context context, ChainTerminator terminator) {
	...
	public void startChainHandlers(int nChainHandlers) {
		...
		for (int j = 0; j < nChainHandlers; ++j) {
			...
			ChainHandler handler = new ChainHandler(context, terminator);
	...
	public void startSeparateChainHandler(Chain chain) {
		ChainHandler handler = new ChainHandler(context, terminator, chain);
	...
	public void doHouseKeeping() {
		...
		if (inactiveHandlers == 0 && activeHandlers < nChainHandlers && chainsToProcess.getNElements() > 0) {
			ChainHandler handler = new ChainHandler(context, terminator);
